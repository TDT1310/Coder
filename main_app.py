...
MAPPING_PATH = Path(base_path) / "Data" / "account_mapping.xlsx"
...
🎯 Your Task:
Compute and assign a score for each of the 3 fraud detection components:
...
Starting with a total score of 0

Adding the score from each component (M-Score, Benford MAD, and Red Flags) to the total

The final total score will be the sum of the three components' scores.

🔹 Scoring Criteria:
M-Score:

≤ -2.22 → add 2 points (no alert)

-2.22 < M ≤ -1.78 → add 1 point (mild warning)

-1.78 → add 0 points (strong warning)

Benford MAD:

≤ 1 → add 2 points

1 < MAD ≤ 1.6 → add 1 point

1.6 → add 0 points

Red Flags (each red flag counts as 1 point):

0 red flags → add 2 points

1 red flag → add 1 point

≥ 2 red flags → add 0 points
...
If any red flag is present, explain it based on the financial data from {year1} and {year2}.

✅ Your Output Should Include:
Total Score: The sum of the M-Score, Benford MAD, and Red Flags' scores.

Brief Summary for Each Component:

M-Score: Provide the value, explain the score, and assess the risk level.

Benford MAD: Provide the value, explain the score, and assess the deviation level.

Red Flags: Number of red flags detected, explain their scoring, and specify what the red flags are.

conclusion with a risk interpretation:
...
Explain how each of the red flags is calculated, its value, and the formula used for detection.

If there is insufficient data to compute the fraud risk score, respond with:
"I do not have enough financial data to compute a fraud risk score."
Include a note specifying what data is lacking for the evaluation.
